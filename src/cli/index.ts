...
function kindFromCommand(cmd: string): CommandKind | null {
  if (cmd === "handoff" || cmd === "explain") return cmd === "explain" ? "explain" : "handoff";
  if (cmd === "pr") return "pr";
  return null;
}

async function runArtifact(args: Args): Promise<number> {
  ...
  if (args.json) {
    ...
    console.error(
      `DevFlow ${kind} · audience=${audience} · ${forceOffline || !model ? "offline" : model} · ${Date.now() - started}ms`
    );
  }
...
function runInit(cwd: string): number {
