...
function detectBase(cwd, preferred) {
    if (preferred) {
...
function shouldRead(path) {
    ...
    if (lower.includes("node_modules/") || lower.includes(".git/"))
        return false;
    if (lower.endsWith("package-lock.json") || lower.endsWith("pnpm-lock.yaml"))
        return false;
    const dot = lower.lastIndexOf(".");
    if (dot >= 0 && SKIP_EXT.has(lower.slice(dot)))
        return false;
    return true;
}
function readSnippet(cwd, relPath, maxBytes) {
...
export function collectGitContext(opts = {}) {
    ...
    const log = run(cwd, ["log", "--oneline", "-n", "15", `${base}..HEAD`], true) ||
    ...
    if (!diff)
        diff = "(no textual diff — untracked-only or binary changes)";
    const changedFiles = listChangedFiles(cwd, base);
    const fileSnippets = [];
    for (const f of changedFiles) {
        if (fileSnippets.length >= maxFiles)
            break;
        if (!shouldRead(f))
            continue;
...
export function defaultOutPath(cwd, kind) {
